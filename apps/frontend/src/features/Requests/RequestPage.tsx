...
export function AllRequestsPage() {
    ...
    const steps = [
        {
            content: <div>Use these tabs to navigate between different service request types.</div>,
            selectorId: TOUR_STEPS_IDS_SERVICE_REQUEST[0].target.replace('[data-tour-id="', '').replace('"]', ''),
            position: "bottom" as const
        },
        {
            content: <div>This card displays all service requests from all departments. You can sort, filter, and view details.</div>,
            selectorId: TOUR_STEPS_IDS_SERVICE_REQUEST[1].target.replace('[data-tour-id="', '').replace('"]', ''),
    ...
    useEffect(() => {
        setSteps(steps);
        const timer = setTimeout(() => setOpenTour(true), 100);
